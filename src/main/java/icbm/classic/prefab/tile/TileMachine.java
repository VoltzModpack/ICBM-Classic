...
public class TileMachine extends TileEntity implements IPacketIDReceiver, IWorldPosition, IPlayerUsing, ITickable, IByteBufWriter, IGuiTile, IEnergySink {
	...
	/**
	 * Toggle to send a {@link #getUpdatePacket()} on the next tick, keep in mind only do this for render data. if the
	 * data is not used by the renderer then send it at the time it is needed. For example, GUI data should be sent to
	 * only GUI users and not everyone.
	 */
