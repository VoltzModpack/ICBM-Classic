...
public abstract class Blast extends Explosion implements IBlastInit, IBlastRestore {
		...
		try {
			if (!this.world().isRemote) {
				...
				if (this instanceof IBlastTickable) {
					if (!this.world().addEntity(new EntityExplosion(this))) {
	...
	/**
	 * Internal call for running post blast code Do not se the entity or blast dead. This is completed in the {@link
	 * #completeBlast()} method.
	 */
	...
	/**
	 * Called to set the position of the blast. Only call this for initialization, anything that happens during world
	 * tick should call {@link #onPositionUpdate(double, double, double)}
	 *
	 * @param posX
	 * @param posY
	 * @param posZ
	...
	/**
	 * Called by doDamageEntity on each entity being damaged. This function should be inherited if something special is
	 * to happen to a specific entity.
	 *
