...
public class BlastHelpersTest {
	...
	class ForEachPosInRadiusGroup {
		...
		void loop3x3() {
			final List<Vector3i> positions = new ArrayList<>(7);
			...
			Assertions.assertEquals(new Vector3i(-1, 0, 0), positions.get(0));
			Assertions.assertEquals(new Vector3i(0, -1, 0), positions.get(1));
			Assertions.assertEquals(new Vector3i(0, 0, -1), positions.get(2));
			Assertions.assertEquals(new Vector3i(0, 0, 0), positions.get(3));
			Assertions.assertEquals(new Vector3i(0, 0, 1), positions.get(4));
			Assertions.assertEquals(new Vector3i(0, 1, 0), positions.get(5));
			Assertions.assertEquals(new Vector3i(1, 0, 0), positions.get(6));
		}
	...
	class ForEachPosInCubeTestGroup {
		...
		void stopAfterFirst() {
			...
			Assertions.assertEquals(new Vector3i(-1, -1, -1), positions.get(0));
		}

		@Test
		@DisplayName("3x3x3 cube - should map all 27 positions in order")
		void loop3x3() {
			final List<Vector3i> positions = new ArrayList<>(27);
			...
			for (int x = -1; x <= 1; x++) {
				for (int y = -1; y <= 1; y++) {
					for (int z = -1; z <= 1; z++) {
						Assertions.assertEquals(new Vector3i(x, y, z), positions.get(index));
						index++;
					}
				}
			}
